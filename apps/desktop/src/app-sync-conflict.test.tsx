...
describe("Sync conflict UI", () => {
  beforeEach(() => {
    ...
    mermaidMocks.render.mockResolvedValue({
      svg: "<svg data-testid=\"sync-conflict-diagram\"></svg>"
    });
  });
  ...
  it("shows conflicts and resolves with local text", async () => {
    ...
    vi.mocked(invoke).mockImplementation((command) => {
      ...
      if (command === "load_page_blocks") {
        return Promise.resolve({
          page_uid: "inbox",
          title: "Inbox",
          blocks: [{ uid: "b1", text: "Remote text", indent: 0 }]
        });
      }
    ...
    const conflictHeading = await screen.findByText(/sync conflicts/i);
    const conflictSection = conflictHeading.closest(".settings-section");
    expect(conflictSection).toBeTruthy();
    const conflictScope = within(conflictSection as HTMLElement);

    expect(
      conflictScope.getByRole("button", { name: /use local/i })
    ).toBeInTheDocument();
    expect(
      conflictScope.getByRole("button", { name: /use remote/i })
    ).toBeInTheDocument();
    expect(conflictScope.getByText("Local text")).toBeInTheDocument();
    expect(conflictScope.getByText("Remote text")).toBeInTheDocument();

    fireEvent.click(conflictScope.getByRole("button", { name: /use local/i }));

    await waitFor(() => {
      const saveCalls = vi
        .mocked(invoke)
        .mock.calls.filter(([cmd]) => cmd === "save_page_blocks");
      expect(saveCalls).toHaveLength(1);
      const [, payload] = saveCalls[0] as [string, { blocks: { text: string }[] }];
      expect(payload.blocks[0]?.text).toBe("Local text");
    });

    await waitFor(() => {
      expect(screen.queryByText(/sync conflicts/i)).not.toBeInTheDocument();
    });
